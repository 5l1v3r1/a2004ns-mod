...
function onDeleteRuleClicked(name, mode, e)
{
	ClickEventPropagater(e);
	var F = parent.document.firewall_fm;
	var F2 = document.firewall_iframe_fm;

	if(confirm(FIREWALL_DEL_CONFIRM_STRING))
	{
		F2.act.value = 'del';
		F2.name.value = name;
		All_Clear_FWForm(mode, parent.document);
		F.clicked_name.value = '';
		F.clicked_bg.value = '';
		F2.mode.value = F.formselect.value;
		var selmenu = parent.parent.frames[3].document.getElementById('fw_select_menu');
		if(selmenu)
...
function onClickedMacSearch(frm_doc)
{
	var F = document.firewall_fm;
	var doc = document;
	...
	var obj = doc.getElementById('MACBOXTD');
	var search_obj = doc.getElementById('MACSEARCHBOXTD');
